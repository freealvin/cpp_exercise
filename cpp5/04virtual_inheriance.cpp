...
class ModernPhone : public MP3, public Camera, public Phone
{
	public:
		...
		ModernPhone(const char* factory, double price):Phone(factory,price), MP3(price), Camera(price), Goods(price+200)
		{//虚继承，虚基类（Goods）的构造函数的参数由底层子类ModernPhone传递,所以ModernPhone getPrice返回的价格为price+200
...
int main()
{
	...
	cout << "The price of modern phone is "<< mp.getPrice() << endl;//500
	return 0;
}
